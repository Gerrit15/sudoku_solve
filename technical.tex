...
managed by the Serde crate and whatever format crate is relevant (likely the CSV crate.) What is likely easiest to do, in concrete terms, is to load in a CSV to a struct
containing a 2D Vector of an enum that has two states: u8 and a Vector of u8, and iterate over that using wave function collapse. Note that Serde will require a custom deserialization.
\end{document}
